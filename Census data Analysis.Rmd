...
ggplot(data = train,aes(x=capital.gain,y=income))+geom_count(aes(colour = hi_cap_gains),position = position_jitter(width = 0,height =0.2),alpha=0.5)+geom_vline(xintercept=Split,color='dodgerblue',lty=2)+scale_x_log10(labels=scales::dollar)
...
train<-train%>%
  mutate(husband_or_wife=relationship %in% c("Husband","Wife"),college_degree=husband_or_wife & education %in% c("Bachelors","Doctorate","Masters","Prof-scgool"),income_dtree=predict(mod_tree1,type='class'))
cg_splits<-data.frame(husband_or_wife=c(TRUE,FALSE),vals=c(5095.5,7073.5))
ggplot(data = train,aes(x=capital.gain,y=income))+geom_count(aes(colour = income_dtree,shape = college_degree),position = position_jitter(width = 0,height = 0.3),alpha=0.5)+facet_wrap(~husband_or_wife)+geom_vline(data = cg_splits,aes(xintercept = vals),color='dodgerblue',lty=2)+scale_x_log10()
printcp(mod_tree1)
train<-train%>%
